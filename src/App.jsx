...
import { 
  ...
  Command, Code, Headphones
} from 'lucide-react';

// --- 深度提炼数据源：100% 覆盖 Paul J Lipsky 知识库 ---
const promptData = [
  {
    id: 'ecommerce',
    nameCn: '🛒 电商与代发货',
  ...
  },
  {
    ...
    prompts: [
      ...
      {
        title: '多智能体团队开发指令',
        description: '让多智能体团队同时工作，打造复杂的定制数据抓取工具。',
        tool: 'MGX Agent',
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-[#111111] font-sans antialiased selection:bg-[#CCFF00] selection:text-black">
      ...
      {/* 🚀 Top Navigation (Ramp Style Minimal) */}
      ...
      <div className="max-w-7xl mx-auto px-6 pt-24 md:pt-32 pb-16 text-center flex flex-col items-center">
        ...
        <p className="text-lg md:text-xl text-gray-500 font-medium max-w-2xl mx-auto leading-relaxed mb-12">
          {globalLang === 'cn' 
            ? "拒绝废话，只提供经过实战检验的高级提示词。从电商自动化到零代码开发，直接复制这些顶级工作流指令，让 AI 真正为你打工。" 
            : "No fluff, just battle-tested, high-quality prompts. From e-commerce to no-code development, copy these top-tier workflow instructions and make AI truly work for you."
          }
        </p>
    ...
    {/* 🚀 Category Filter Pills (Now fully translated!) */}
    <div className="max-w-[90rem] mx-auto px-6 mb-12">
      {/* 修复滑动：移除了 md:justify-center，始终保持 justify-start */}
      <div className="flex overflow-x-auto gap-3 pb-4 hide-scrollbar items-center justify-start">
        <button
      ...
      {currentPrompts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {currentPrompts.map((prompt, idx) => (
              <div 
                key={idx}
                className="relative w-full h-[450px] bg-[#F4F4F5] rounded-[32px] overflow-hidden group transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] flex flex-col"
              >
                ...
                <div className="p-8 flex flex-col h-full z-10 relative">
                  ...
                  <div className="mb-5 flex-shrink-0">
                     <div className="flex items-center gap-3 mb-4">
                       <span className="text-[11px] font-extrabold uppercase tracking-widest text-[#111] bg-[#CCFF00] px-3 py-1.5 rounded-lg shadow-sm">
                         {prompt.tool}
                       </span>
                     </div>
                     <h3 className="text-[22px] font-extrabold text-[#111111] leading-[1.2] tracking-tight line-clamp-2">
                       {prompt.title}
                     </h3>
                  </div>

                  {/* 核心区：展示提示词正文 */}
                  <div className="flex-1 bg-white rounded-2xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 overflow-y-auto hide-scrollbar relative">
