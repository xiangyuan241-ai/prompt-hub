...
const App = () => {
  ...
  // ⚠️ 修复点 1：去除了外层导致手机端滑动卡死的 `overflow-x-hidden` 和 `w-full`
  return (
    <div className="min-h-screen bg-white text-[#111111] font-sans antialiased selection:bg-[#CCFF00] selection:text-black">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-24 md:pt-32 pb-16 text-center flex flex-col items-center">
        <h1 className="text-[2.5rem] sm:text-7xl md:text-[5.5rem] font-extrabold tracking-tighter leading-[1.15] text-[#111111] mb-6">
          {globalLang === 'cn' ? '我们只写' : 'We only write'} <br />
          <span className="bg-[#CCFF00] px-4 pb-2 pt-1 rounded-2xl inline-block mt-2 shadow-sm whitespace-nowrap">
      ...
      {/* 🚀 Category Filter Pills - ⚠️ 修复点 2：使用 w-max 和内边距实现完美的横向滑动及留白 */}
      <div className="w-full mb-12 overflow-x-auto hide-scrollbar">
        <div className="flex items-center gap-3 px-6 pb-4 w-max min-w-full md:justify-center mx-auto max-w-[90rem]">
          <button
